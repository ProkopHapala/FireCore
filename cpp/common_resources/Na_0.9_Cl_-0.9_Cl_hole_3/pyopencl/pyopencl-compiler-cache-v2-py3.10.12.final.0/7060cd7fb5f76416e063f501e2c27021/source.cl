...
__kernel void poissonW(
    ...
    const float kz = ((iz <= nz2) ? iz : iz - nz) * freq_z;
        ...
        // Print k-points (only for specific points to avoid flooding)
    // if (ix == 0 && iy == 0) {
    //     printf("k-point at iz=%d: kx=%f, ky=%f, kz=%f\n", iz, kx, ky, kz);
    // }
    if (iz == 0) {  // Only print for first z-layer to avoid too many prints
    printf("k-point at (ix=%d,iy=%d,iz=%d): kx=%f, ky=%f, kz=%f\n", 
           ix, iy, iz, kx, ky, kz);
}
...
__constant int pyopencl_defeat_cache_2e91101636fa4b779d06f428f2f5d04e = 0;