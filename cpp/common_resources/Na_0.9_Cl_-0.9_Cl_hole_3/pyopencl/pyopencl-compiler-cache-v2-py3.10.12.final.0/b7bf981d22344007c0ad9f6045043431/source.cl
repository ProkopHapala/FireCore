...
__kernel void slabPotential( 
    ...
    // More comprehensive debug print
    if(iz==0 && ix==0 && iy==0){
        printf("DEBUG: slabPotential_zyx params: dz=%f Vol=%f dVcor=%f Vcor0=%f\n", 
               params.x, params.y, params.z, params.w);
        printf("DEBUG: Vcor_z=%f at (ix,iy,iz)=(%d,%d,%d)\n", 
               Vcor_z, ix, iy, iz);
    }
...
__kernel void slabPotential_zyx(
    ...
    if(iz==0 && ix==0 && iy==0){
        ...
        printf("DEBUG: Vcor_z=%f at (ix,iy,iz)=(%d,%d,%d)\n",
               Vcor_z, ix, iy, iz);
        printf("DEBUG: Grid dimensions ng=(%d,%d,%d,%d)\n",
               ng.x, ng.y, ng.z, ng.w);
    }

    // Print values at different z positions
    if(ix==0 && iy==0 && (iz==0 || iz==ng.w/2 || iz==ng.w-1)){
        printf("DEBUG: slabPotential_zyx at iz=%d: Vcor_z=%f, z_pos=%f\n",
               iz, Vcor_z, iz*dz);
    }
    ...
    // Debug print for index calculations and values
    if(ix==0 && iy==0 && (iz==0 || iz==ng.w/2 || iz==ng.w-1)){
        printf("DEBUG: slabPotential_zyx indices at iz=%d: i=%d, j=%d, nz_=%d\n",
               iz, i, j, nz_);
        printf("DEBUG: slabPotential_zyx values at iz=%d: Vin[i]=%f, Vout[i]=%f\n",
               iz, Vin[i], Vin[i] + Vcor_z);
    }

    Vout[i] = Vin[i] + Vcor_z;
    
    // Print for all threads at z=0 (original debug print)
    if(iz==0) printf("Vcor_z=%f at (ix,iy)=(%d,%d)\n", Vcor_z, ix, iy);
...
__constant int pyopencl_defeat_cache_c3f0cb6b2344450abc2ee05842abb6d2 = 0;