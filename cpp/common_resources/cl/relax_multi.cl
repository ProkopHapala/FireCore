...
__kernel void updateAtomsMMFFf4(
    ...
    // if( (iS==0)&&(iG==19) ){ printf( "GPU:fe.3[iS=%i,iG=%i](%g,%g,%g,%g) \n",iS,iG, fe.x,fe.y,fe.z,fe.w ); }
        ...
        if( cons.w > 0.0f ){
            // printf( "GPU:sys[%i]atom[%i] constr(%g,%g,%g|%g) constrK(%g,%g,%g|%g)\n", iS, iG, cons.x,cons.y,cons.z, cons.w,   cK.x,cK.y,cK.z,cK.w  );
...
__kernel void analyzeReplicaMMFF(
    ...
    __global float4*  apos,          // [nSys*(natoms+nnode)]
    __global float4*  avel,          // [nSys*(natoms+nnode)]
    ...
    const int nvec   = n.x + n.y;
    const int nPairs = n.z;
    const int i0v    = isys*nvec;

    float v2sum = 0.0f;
    int nMobile = 0;
    for(int ia=0; ia<natoms; ia++){
        const float3 v = avel[i0v + ia].xyz;
        v2sum += dot(v,v);
        nMobile++;
    }
    ...
    for(int ip=0; ip<nPairs; ip++){
        ...
        if((p.x>=0) && (p.x<natoms) && (p.y>=0) && (p.y<natoms)){
            const float3 d = apos[i0v + p.y].xyz - apos[i0v + p.x].xyz;
            r = length(d);
        }
        const int io = i0p + ip;
        anaPairDists[io] = r;
