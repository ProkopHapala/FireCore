...
int rayPickBond( const Vec3d& ray0, const Vec3d& hRay, int nb, Func func, const double R=0.5, const bool byCenter=false ){
    double dist_min =  1e+300;
    int    imin = -1;
    for(int i=0; i<nb; i++){
        Vec3d pa,pb; func(i,pa,pb);
        if(byCenter){
            double ti = raySphere( ray0, hRay, R, (pa+pb)*0.5 );
