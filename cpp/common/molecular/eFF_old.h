...
void printEnergies(){
    printf( "Etot (eFF_old.h) %g | Ek %g Eee,p(%g,%g) Eae,p(%g,%g) Eaa %g \n", Etot, Ek, Eee,EeePaul, Eae,EaePaul, Eaa );
}
...
int Eterms2str(char* str){
    ...
    return sprintf( str, "Etot (eFF_old.h 2) %3.3f|%3.3f Ek %3.3f Eee,P(%3.3f,%3.3f) Eae,P(%3.3f,%3.3f) Eaa %g )\n", Etot, Eorbs+Eaa, Ek, Eee, EeePaul, Eae, EaePaul, Eaa );
}

int orb2str(char* str, int ie){
    return sprintf( str, "e[%i] E %7.3f s %5.2f  p(%5.2f,%5.2f,%5.2f)  f(%5.2f,%5.2f,%5.2f) \n", ie, eE[ie], esize[ie], epos[ie].x,epos[ie].y,epos[ie].z,    fsize[ie],eforce[ie].x,eforce[ie].y,eforce[ie].z  );
}
