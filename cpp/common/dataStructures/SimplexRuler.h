...
inline Vec2d trinagleDeriv( Vec3d hs ){
    ...
    double dFdx = dFda;
    double dFdy =  -0.57735026919*dFda + 1.15470053839*dFdb;
...
class SimplexRuler{
    ...
    //constexpr const static Vec2d lvecs[3]={ (Vec2d){1.0,0.0},(Vec2d){-0.5,0.86602540378},(Vec2d){-0.5,-0.86602540378} };
    constexpr const static Vec2d lvecs[3]={ (Vec2d){0.0,1.0},(Vec2d){0.86602540378,-0.5},(Vec2d){-0.86602540378,-0.5} };
    ...
    inline void tilePoint( const Vec2i& ind, bool s, Vec2d& p ) const {
        nodePoint( ind, p );
        if( s ){ p.x+=step; p.y+=0.57735026919*step; }else{ p.x+=0.5*step; p.y+=0.28867513459*step;  }
    };
    ...
    inline void setStep( double step_     ){ step = step_; invStep = 1.0/step; };

    void rayStart( Vec2d ray0, Vec2d hray ){
        // dlat/dt
        ray_d.a = hray.dot( { 0.0        ,1.15470053839*invStep} );
        ray_d.b = hray.dot( { 1.0*invStep,0.57735026919*invStep} );
        ray_d.c = hray.dot( {-1.0*invStep,0.57735026919*invStep} );
        // pos in grid coords
        ray_p.a = ray0.dot( { 0.0        ,1.15470053839*invStep} );
        ray_p.b = ray0.dot( { 1.0*invStep,0.57735026919*invStep} );
        ray_p.c = ray0.dot( {-1.0*invStep,0.57735026919*invStep} );
