...
function evalV( ps::Matrix{Float64}, qs::Vector{Float64}, pvs::Matrix{Float64})::Vector{Float64}
    nq = size(ps, 1)
    nV = size(pvs,1)
    V  = zeros(Float64, num_targets)
    for i in 1:nV
        pv = pvs[i,:]
        Vi:Float64 = 0.0
...
function fit_charges_lm( ps::Matrix{Float64}, pVs::Matrix{Float64}, Vref::Vector{Float64}, q0s::Union{Nothing, Vector{Float64}}=nothing, Qtot::Float64=1.0 )
    model = (qs, x) -> evalV(ps, qs, pVs)
    if q0s === nothing
        q0s = ones(Float64, size(ps, 1)).*(Qtot/size(ps,1))
    end
    result = curve_fit( model, q0s, target_points, Vref )
    qs_out = result.param
    return qs_out
end

function generate_circle_points(center::Tuple{Float64, Float64}, Rs::Vector{Float64}, nPhi::Int)::Matrix{Float64}
    nr = length(Rs)
    ntot = nr * nPhi
    cx, cy = center
...
function create_circular_mask(grid::Grid2D, center::Tuple{Float64, Float64}, r_min::Float64, r_max::Float64)::Array{Bool, 2}
    # Unpack grid properties
    x0, y0 = grid.g0
    dx, dy = grid.dg
    nx, ny = grid.n
...
Rs = [1.0, 3.0]
pVs = generate_circle_points( Tuple(pq), Rs, 32 )

mask = create_circular_mask( grid, Tuple(pq), 1.0, 3.0 )
...
scatter!(plt_V, pVs[:, 1], pVs[:, 2], marker=:circle, color=:black, label="Charge Points", markersize=3)
display(plt_V);