...
from plot_utils import plot_SV, plot1D, plotFunctionApprox
from basis_utils import morse_potential, gen_morse_prms, gen_morse_curves
...
if __name__ == "__main__":
    ...
    print(f"\nGenerating {n} Morse potential samples...")
    # Generate parameters first, then curves
    prms_list = gen_morse_prms(n, a_rng = (0.8, 2.5), r0_rng = (2.5, 4.0))
    ...
    plot_SV(singular_values, K_opt=num_optimal_basis)
    plot1D(z_values, optimal_basis_cols.T, title=f'{num_optimal_basis} Optimal Basis Functions Derived by SVD' )
