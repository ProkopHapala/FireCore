"""
run_ES.py  –  CPU entropic-spring thermodynamic integration.

Runs compute_Free_energy() over a lambda grid and compares the result
against the analytical Gaussian-chain free energy.  Produces one data file
and one PNG plot.

Usage (from examples/tFreeEnergy/):
    python3 run_ES.py [--nbStep N] [--nMDsteps N] [--nEQsteps N] [-T K]
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="CPU TI for entropic spring vs analytical Gaussian-chain reference."
    )
    parser.add_argument("--xyz_name",    default="../tMMFF/data/entropic_spring_20.xyz", help="Path to XYZ file")
    parser.add_argument("--constr_name", default="../tMMFF/data/entropic_spring_20.cons", help="Path to .cons file")
    ...
    parser.add_argument("--nbStep",      type=int,   default=100, help="Number of lambda windows")
    parser.add_argument("--nMDsteps",    type=int,   default=100000, help="Production MD steps per lambda window")
    parser.add_argument("--nEQsteps",    type=int,   default=10000, help="Equilibration steps per lambda window")
    ...
    # Shared stem for output files, e.g. "entropic_spring_20_TI_cpu_T300K"
    out_stem = f"{xyz_name.stem}_TI_cpu_T{args.temperature:.0f}K"
    ...
    mmff.init(
        ...
        sDihedralTypes="../../cpp/common_resources/DihedralTypes.dat",
        bMMFF=True, bEpairs=False,
        nPBC=(0, 0, 0),
    )

    # --- Run CPU TI ---
    mmff.compute_Free_energy(
        args.lamda1, args.lamda2, dc=dc,
        nbStep=args.nbStep, nMDsteps=args.nMDsteps, nEQsteps=args.nEQsteps,
        t_damp=args.t_damp, T=args.temperature, dt=args.dt,
    )

    # --- Load raw TI output written by C++ ---
    raw_data = np.loadtxt(results_dir / "TI_plot_ES.dat", skiprows=1)
    if raw_data.ndim == 1:
    ...
    ax.plot(lam, ti,  label="CPU TI")
    ax.fill_between(lam, ti - sigma, ti + sigma, alpha=0.25, label="±1σ")
    ax.plot(lam, ref, "--", label="Gaussian chain (analytical)")
    ax.set_xlabel("λ [Å]")
    ax.set_ylabel("ΔF [eV]")
    ax.set_title(f"Entropic spring TI — {xyz_name.stem},  T = {args.temperature:.0f} K")
    ax.legend()
    fig.tight_layout()
    fig.savefig(out_png, dpi=150)
    plt.close(fig)

    print(f"ΔF (CPU TI, last window) : {ti[-1]:.6f} eV")
