...
! Find the unit vector in sigma direction.
           if (y .lt. 1.0d-05) then
            ...
            sighat(2) = 0.0d0
            sighat(3) = 1.0d0
            write (*,*) ' There is an error here in Dassemble_ca_snxc_3c.f '
            write (*,*) ' r1 = r2!!!! BAD!!!! '
           else
            sighat(:) = r21(:)/y
           end if
            ...
            do jssh = 1, nssh(in2)
             sm = sm_mat(issh,jssh,mneigh,iatom)
              ...
              if (abs(sm) .gt. xc_overtol) then 
               do ix = 1,3
                spm(ix) = spm_mat(ix,issh,jssh,mneigh,iatom)
                avrhop_b (ix,issh,jssh) = avrhop_b (ix,issh,jssh)            &
     &           + (sm*rhop_3cb(ix,issh,jssh) - rho_3c(issh,jssh)*spm(ix))   &
     &            /(sm*sm)
                avrhop_c (ix,issh,jssh) = avrhop_c (ix,issh,jssh)            &
     &           + (sm*rhop_3cc(ix,issh,jssh) + rho_3c(issh,jssh)*spm(ix))   &
     &            /(sm*sm)
               end do
              end if
            end do
...
! ===========================================================================
        ...
        return
      end subroutine Dassemble_ca_snxc_3c
