...
!         end if
         ...
         if (divide) then
           call cheevd('V', 'U', norbitals, zzzz, norbitals, slam, work, lwork, rwork , lrwork, iwork, liwork, info ) 
         else
           call cheev ('V', 'U', norbitals, zzzz, norbitals, slam, xxxx, lwork, rwork , info)
         end if
...
! after it is combined with overlap.
         ...
         call cgemm ('N', 'C', norbitals, norbitals, norbitals_new, a1, zzzz, norbitals, zzzz, norbitals, a0, xxxx, norbitals)
...
! ****************************************************************************
! Set M=H*(S^-.5)
        call chemm ( 'R', 'U', norbitals, norbitals, a1, xxxx, norbitals, yyyy, norbitals, a0, zzzz, norbitals )

! Set Z=(S^-.5)*M
        call chemm ( 'L', 'U', norbitals, norbitals, a1, xxxx, norbitals, zzzz, norbitals, a0, yyyy, norbitals )
...
! Eigenvectors are needed to calculate the charges and for forces!
        if (divide) then
          call cheevd('V', 'U', norbitals, yyyy, norbitals, eigen, work, lwork, rwork , lrwork, iwork, liwork, info ) 
        else
          call cheev ('V', 'U', norbitals, yyyy, norbitals, eigen, zzzz, lwork, rwork, info)
        end if
        if (info .ne. 0) call diag_error (info, 0)
...
! yyyy = Hamiltonian eigenvectors in the MO basis
        ...
        call chemm ( 'L', 'U', norbitals, norbitals, a1, xxxx, norbitals, yyyy, norbitals, a0, zzzz, norbitals )
