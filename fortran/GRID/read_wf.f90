...
! Perform some checks
         ...
         if (rcutoffwf .le. (rcutoff(ispec,issh)/abohr - 1.0d-2) .or.  rcutoffwf .ge. (rcutoff(ispec,issh)/abohr + 1.0d-2)) then
            write (*,*) ' rcutoffwf = ', rcutoffwf, ' rcutoff = ',   rcutoff(ispec,issh)/abohr
         ...
         if (xnoccwf .ne. Qneutral(issh,ispec)) then
            write (*,*) ' xnoccwf = ', xnoccwf, ' xnoccin = ', Qneutral(issh,ispec)
...
! Read in the points
         ...
         do ipoint = 1, mesh - iremainder, 4
            read (17,300) psitemp(ipoint), psitemp(ipoint+1), psitemp(ipoint+2), psitemp(ipoint+3)
         end do
...
! Check normalization
         ...
         do ipoint = 1, mesh
            if (ipoint .ne. 1 .or. ipoint .ne. mesh) then
               sum = sum + drr_wf(issh,ispec)*    rr_wf(ipoint,issh,ispec)**2 * wf(ipoint,issh,ispec)**2
            else
