...
/// Config: scripts/small_symmetric_nc.json → OUT_small_nc/<material>/<id>/{init.mol2,init.xyz,gen_params.json}
...
function loadCfg(p) {
    const cfgPath = p ? path.resolve(p) : path.join(REPO, 'scripts/small_symmetric_nc.json');
...
function main() {
    ...
    const outRoot = path.resolve(REPO, cfg.outDir || 'OUT_small_nc');
