...
PROGRESS_REPORT_KEYWORDS = [
    "session notes", "parity recap", "progress update", "debugging",
    ...
    "race condition", "segfault", "nan", "mismatch", "discrepancy",
...
def classify_file(filepath: str, text: str) -> Tuple[str, int]:
    ...
    # LLM chat is definitive if score is high enough
...
def main():
    ...
    for p in paths:
        ...
        if is_meta_doc(p):
            ...
            confidence = 95
            scores = {}
        # 2. Config filter (SKILL.md, workflows, .windsurf, .cursor, etc.)
        elif is_config(p):
            doc_type = "config"
            confidence = 95
            scores = {}
    ...
    TYPE_ORDER = [
        "meta_doc", "config", "user_guide", "api_doc", "design_doc",
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        f.write("- **config** — Agent/IDE configuration: SKILL.md, workflows, plans, .windsurf, .cursor, .devin\n")
