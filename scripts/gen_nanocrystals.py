#!/usr/bin/env python3
"""Nanocrystal generator — Python CLI. Sphere cut: native; Miller planes + bridges: node scripts/gen_nanocrystals.mjs."""
import argparse
import os
import subprocess
import sys
...
def run_node_planes(args):
    cmd = ['node', os.path.join(REPO, 'scripts', 'gen_nanocrystals.mjs'),
...
def main():
    ...
    ap.add_argument('--outDir', dest='out_dir', default=os.path.join(REPO, 'OUT_nanocrystals_py'))
