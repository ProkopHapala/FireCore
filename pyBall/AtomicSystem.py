...
# ========= Adding Electron Pairs
    ...
    def add_electron_pairs(self, distance=0.5):
        """
        ...
        for i, ename in enumerate(self.enames):
            ...
            if nep > 0:
                self.make_epair_geom(i, npi, nsigma, distance=distance)
    ...
    def make_epair_geom(self, i, npi, nb, distance=0.5 ):
        """
        ...
        if npi == 0:
            if nb == 3:   # like NH3
                ...
                self.place_electron_pair(i, base, distance=distance)
            elif nb == 2: # like H2O
                ...
                self.place_electron_pair(i, ep1, distance=distance)
                self.place_electron_pair(i, ep2, distance=distance)
        elif npi == 1:
            ...
            if nb == 2: # like =N-
                ...
                self.place_electron_pair(i, m_c*-1., distance=distance)
            elif nb == 1:  # like =O 
                #print( f"make_epair_geom() like =O {self.enames[i]}    ia: {i} npi: {npi} nb: {nb}" )
                _neighs = list(self.ngs[i].keys())
                m_b = self.get_atomi_pi_direction( _neighs[0] ) if _neighs else np.array([0.0, 0.0, 1.0])
                m_c = au.normalize(np.cross( v1, m_b ))
                self.place_electron_pair(i, v1*-0.5 + m_c*0.86602540378, distance=distance)
                self.place_electron_pair(i, v1*-0.5 - m_c*0.86602540378, distance=distance)
        elif npi == 2:
            if nb == 1:  # like N in HCN (triple bond)
                self.place_electron_pair(i, -v1, distance=distance)
