...
typedef void (*P_assembleH  )(int,int,double*);
typedef void (*P_solveH     )(double*,int);
typedef void (*P_getPointer   )(double**);
//firecore_getPointer_wfcoef( bbnkre_ ) bind(c, name='firecore_getPointer_wfcoef')
...
class Lib{ public:
    ...
    inline int loadLib( const char* fullname ){
        ...
        assembleH  = (P_assembleH)dlsym(lib_handle, "firecore_assembleH");
        if ((error = dlerror())){ printf("%s\n", error); preinit=0; exit(0); }
        solveH     = (P_solveH)dlsym(lib_handle, "firecore_solveH");
        if ((error = dlerror())){ printf("%s\n", error); preinit=0; exit(0); }
        getPointer_wfcoef     = (P_getPointer)dlsym(lib_handle, "firecore_getPointer_wfcoef");
        if ((error = dlerror())){ printf("%s\n", error); preinit=0; exit(0); }
...
/*
    void applyCharges( Vec3d* aREQs, bool bDiff=false ){
        ...
        for(int iq=0; iq<nqm; iq++){
            int im      = imms[iq];
            //if(params) aREQs[im].z = charges[iq];
        }
