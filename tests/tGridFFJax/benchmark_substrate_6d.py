#!/usr/bin/env python3
"""6D apples-to-apples benchmark — substrate-agnostic.

Pipeline
--------
1. Build a 6D rigid pose batch (u, v, z, quaternion) over the surface unit cell.
2. Evaluate the teacher (MACE / generic ASE calc / precomputed npz) at every pose.
...
4. Fit GridFF parameters on the train split (Stage 1 PLQ + Stage 2 raw 1/r^6 lstsq,
   energy-only or joint E + F).
5. Re-evaluate the FITTED GridFF at the SAME poses.
6. Diagnostics (per split, per z-bin, per site, per channel) + 11 plots.

Substrate classes
-----------------
  --substrate-class metal   (Ag, Au, Cu — default; uses CHGCAR + LOCPOT)
  --substrate-class ionic   (NaCl, KBr — uses point-charge xyz + Ewald slab)

Teacher backends
----------------
  --teacher-kind madsurf       MAD-SURF MACE (coin-metal only)
  --teacher-kind generic_calc  Any ASE calculator (MACE, ANI, NEP, AIMNet, …);
                               specify --teacher-module + --teacher-callable.
  --teacher-kind precomputed   Read energies/forces from --teacher-npz; the user
                               supplies this from VASP/Psi4/Gaussian/etc. offline.

Density backends
----------------
  --density-kind vasp_volumetric   CHGCAR / LOCPOT (VASP)        [metal default]
  --density-kind cube              Gaussian cube                  (Psi4/Gaussian/...)
  --density-kind surface_xyz       Point-charge xyz               [ionic default]

Cache
-----
Re-running with the same flags reuses the teacher npz from a previous run in
the same --out-dir. --rebuild-teacher forces re-evaluation. --load-apples PATH
skips teacher entirely and refits an existing apples_dataset.npz.

Typical CLI
-----------
  # Ag/CHONH2 (the original 16 meV recipe path):
  python benchmark_substrate_6d.py --substrate-class metal \\
      --adsorbate CHONH2 --device cuda \\
      --out-dir tests/tGridFFJax/runs/ag6d_chonh2

  # NaCl ionic, MACE teacher (smoke):
  python benchmark_substrate_6d.py --substrate-class ionic \\
      --density-xyz cpp/common_resources/xyz/NaCl_1x1_L1_jax.xyz \\
      --adsorbate H2O --device cpu \\
      --teacher-kind generic_calc --teacher-module mace.calculators \\
      ...
      --out-dir tests/tGridFFJax/runs/nacl_ionic_smoke

  # NaCl with user's Psi4 data (once npz is prepared):
  python benchmark_substrate_6d.py --substrate-class ionic \\
      --density-xyz cpp/common_resources/xyz/NaCl_1x1_L1_jax.xyz \\
      --adsorbate H2O \\
      --teacher-kind precomputed --teacher-npz path/to/psi4_dataset.npz \\
...
from pyBall.gridff_jax.fit.optimize import FitResult
...
from pyBall.gridff_jax.substrate_classes import (
    apply_substrate_class, get_substrate_class_recipe,
)
...
# Workstation-specific defaults are pulled from environment variables so the
# script is portable. Set them in your shell init or pass --chgcar/--locpot/
# --model-path explicitly. The MAD-SURF MACE default does have a sensible
# in-repo fallback (it's tracked under tests/tGridFFJax/mad-surf_data/) — for
# CHGCAR/LOCPOT there is no in-repo default because they belong to per-user
# DFT projects.
import os as _os
...
DEFAULT_MODEL = _os.environ.get("GRIDFF_JAX_MODEL_PATH") or (
    str(_REPO_MACE_DEFAULT) if _REPO_MACE_DEFAULT.exists() else None
)
...
def parse_args():
    ...
    # Substrate class — determines builder_mode, FeatureToggles, defaults
    p.add_argument("--substrate-class", choices=["metal", "ionic"], default="metal",
                   help="Conductive metal (Ag/Au/Cu, default) or ionic (NaCl/KBr).")

    # Density backend
    p.add_argument("--density-kind", default=None,
                   choices=[None, "vasp_volumetric", "cube", "surface_xyz"],
                   help="Override the density backend choice. Default: class-dependent "
                        "(metal→vasp_volumetric, ionic→surface_xyz).")
    p.add_argument("--chgcar", default=DEFAULT_CHGCAR,
                   help="Density file (VASP CHGCAR or Gaussian cube). Reused by both "
                        "vasp_volumetric and cube backends as the 'main density' input.")
    p.add_argument("--locpot", default=DEFAULT_LOCPOT,
                   help="Optional second volumetric file (LOCPOT or potential cube). "
                        "Ignored by ionic class.")
    p.add_argument("--density-xyz", default=None,
                   help="Surface xyz with point charges (for surface_xyz / ionic).")

    # Teacher backend
    p.add_argument("--teacher-kind", default="madsurf",
                   choices=["madsurf", "generic_calc", "precomputed", "synthetic"],
                   help="madsurf=MACE coin-metal model; generic_calc=any ASE calc; "
                        "precomputed=load energies/forces from npz.")
    p.add_argument("--model-path", default=DEFAULT_MODEL,
                   help="Path to the teacher model (MACE, ANI, NEP, ...).")
    p.add_argument("--teacher-module", default=None,
                   help="Python module that exports the calculator class "
                        "(for --teacher-kind generic_calc). E.g. 'mace.calculators'.")
    p.add_argument("--teacher-callable", default=None,
                   help="Calculator class/factory name in --teacher-module. "
                        "E.g. 'MACECalculator'.")
    p.add_argument("--teacher-npz", default=None,
                   help="Precomputed teacher data (npz) for --teacher-kind precomputed.")
    ...
    # Fit — most defaults come from the SUBSTRATE CLASS recipe; pass --foo
    # to override the class default for that knob.
    p.add_argument("--london-damp-d0", type=float, default=None,
                   help="Override class default for London Fermi damping d0.")
    p.add_argument("--london-damp-width", type=float, default=None,
                   help="Override class default for London Fermi damping width.")
    p.add_argument("--use-pairwise-c6", type=lambda s: s.lower() in ("true", "1", "yes"),
                   default=None,
                   help="Override class default for pairwise C6 channel (true/false).")
    p.add_argument("--two-stage-c6", type=lambda s: s.lower() in ("true", "1", "yes"),
                   default=None,
                   help="Override class default for two-stage C6 fit (true/false).")
    p.add_argument("--fit-c6-coeff", type=lambda s: s.lower() in ("true", "1", "yes"),
                   default=None,
                   help="Override class default for fit_c6_coeff (true/false).")
    ...
    p.add_argument("--force-weight", type=float, default=1.0,
                   help="Weight on force residuals in joint lstsq.")

    # Student
    p.add_argument("--student-chunk", type=int, default=64)
    # BooleanOptionalAction lets the user disable JAX (--no-prefer-jax) to
    # debug numerical parity against NumPy, isolate JAX compile issues, or
    # force CPU-only reproducibility.
    p.add_argument("--prefer-jax", action=argparse.BooleanOptionalAction, default=True,
                   help="Use the JAX backend for grid assembly + Stage-1 fit. "
                        "Pass --no-prefer-jax to force NumPy.")
...
def build_run_config(args) -> RunConfig:
    ...
    # --- Substrate class sets builder_mode, toggles, density-kind default, teacher-tile default ---
    # User-explicit CLI values for density-kind / teacher-tile take precedence over class defaults.
    user_explicit_density_kind = args.density_kind is not None
    apply_substrate_class(
        cfg, args.substrate_class,
        override_density_kind=not user_explicit_density_kind,
        override_teacher_tile=False,   # teacher-tile is parsed below from the CLI string
        verbose=True,
    )
    if user_explicit_density_kind:
        cfg.density_backend.kind = args.density_kind  # type: ignore[assignment]

    # --- Density inputs ---
    if cfg.density_backend.kind in ("vasp_volumetric", "cube"):
        cfg.density_backend.chgcar_path = args.chgcar
        cfg.density_backend.locpot_path = args.locpot
    elif cfg.density_backend.kind == "surface_xyz":
        if not args.density_xyz:
            raise ValueError("--substrate-class ionic / --density-kind surface_xyz requires --density-xyz")
        cfg.density_backend.xyz_path = args.density_xyz
    else:
        # other backends — set whatever the user gave, let the backend validate
        cfg.density_backend.chgcar_path = args.chgcar
        cfg.density_backend.locpot_path = args.locpot

    # --- Teacher backend ---
    cfg.teacher_backend.kind = args.teacher_kind
    ...
    if args.teacher_kind == "generic_calc":
        if not args.teacher_module or not args.teacher_callable:
            raise ValueError("--teacher-kind generic_calc needs --teacher-module and --teacher-callable")
        cfg.teacher_backend.model_module = args.teacher_module
        cfg.teacher_backend.model_callable = args.teacher_callable
    elif args.teacher_kind == "precomputed":
        if not args.teacher_npz:
            raise ValueError("--teacher-kind precomputed needs --teacher-npz")
        cfg.teacher_backend.metadata = {"npz_path": args.teacher_npz}

    # --- Teacher tile (parse CLI string; overrides class default) ---
    ...
    # --- Grid scalars not tied to substrate class ---
    cfg.grid.alpha_morse = float(args.alpha_morse)
    cfg.grid.c6_rcut = float(args.c6_rcut)

    # --- Optional CLI overrides of class-owned physics knobs ---
    # If the user passes --london-damp-d0 / --london-damp-width / --c6 toggle
    # explicitly, those win over the substrate-class default. The defaults
    # already came from apply_substrate_class().
    if args.london_damp_d0 is not None:
        cfg.grid.london_damping_d0 = float(args.london_damp_d0)
    if args.london_damp_width is not None:
        cfg.grid.london_damping_width = float(args.london_damp_width)
    if args.use_pairwise_c6 is not None:
        cfg.grid.use_pairwise_c6 = bool(args.use_pairwise_c6)
    if args.two_stage_c6 is not None:
        cfg.training.two_stage_c6 = bool(args.two_stage_c6)
    if args.fit_c6_coeff is not None:
        cfg.training.fit_c6_coeff = bool(args.fit_c6_coeff)

    cfg.hybrid_model.use_req_plq = True

    # Fit-pipeline knobs (substrate-class-agnostic; toggles like fit_c6_coeff
    # already set above from the class recipe)
...
#  Pose helpers
# ---------------------------------------------------------------------------
def pose_signature(poses: PoseBatch, adsorbate_name: str,
                   teacher_kind: str = "", model_path: str = "",
                   teacher_tile=(0, 0), interaction_energy: bool = True,
                   density_kind: str = "", density_paths: tuple = ()) -> str:
    """Hash that fingerprints not only the poses but the full upstream
    identity of the data — teacher, model, tiling, interaction-energy mode,
    density backend and its files. Without these, a teacher-kind switch can
    silently reuse stale cached data with the wrong labels."""
    h = hashlib.sha256()
    ...
    if poses.adsorbate.charges is not None:
        h.update(b"|q|")
        h.update(np.ascontiguousarray(poses.adsorbate.charges, dtype=np.float64).tobytes())
    h.update(b"|teacher|"); h.update(str(teacher_kind).encode())
    h.update(b"|model|");   h.update(str(model_path).encode())
    h.update(b"|tile|");    h.update(str(tuple(int(x) for x in teacher_tile)).encode())
    h.update(b"|inter|");   h.update(str(bool(interaction_energy)).encode())
    h.update(b"|dkind|");   h.update(str(density_kind).encode())
    for pth in density_paths:
        h.update(b"|dpath|"); h.update(str(pth or "").encode())
    return h.hexdigest()[:16]


def _signature_from_cfg(poses, adsorbate_name: str, cfg) -> str:
    """Convenience: build a cache signature from RunConfig + adsorbate."""
    dpaths = (
        getattr(cfg.density_backend, "chgcar_path", None),
        getattr(cfg.density_backend, "locpot_path", None),
        getattr(cfg.density_backend, "xyz_path", None),
        getattr(cfg.density_backend, "volumetric_npz_path", None),
    )
    return pose_signature(
        poses=poses, adsorbate_name=adsorbate_name,
        teacher_kind=cfg.teacher_backend.kind,
        model_path=str(cfg.teacher_backend.model_path or ""),
        teacher_tile=tuple(cfg.teacher_backend.teacher_tile),
        interaction_energy=bool(cfg.teacher_backend.interaction_energy),
        density_kind=cfg.density_backend.kind,
        density_paths=dpaths,
    )
...
def stratified_split_indices(poses: PoseBatch, val_frac: float, test_frac: float,
    ...
    if not 0.0 <= val_frac < 1.0 or not 0.0 <= test_frac < 1.0:
        raise ValueError(
            f"val_frac and test_frac must be in [0, 1); got val_frac={val_frac}, "
            f"test_frac={test_frac}"
        )
    if val_frac + test_frac >= 1.0:
        raise ValueError(
            f"val_frac + test_frac must be < 1.0 to leave train poses; got "
    ...
    for s in np.unique(stratum):
        ...
        rng.shuffle(members)
        n_s = len(members)
        # Zero-fraction means zero entries — do NOT force a floor of 1.
        # When the user asks for --val-fraction 0 --test-fraction 0, every
        # pose goes to train; the framework no longer fabricates splits.
        n_test = int(round(n_s * test_frac)) if test_frac > 0.0 else 0
        n_val = int(round(n_s * val_frac)) if val_frac > 0.0 else 0
        # Bound so we always leave at least one train sample per stratum.
        n_train = n_s - n_test - n_val
        if n_train < 1:
            excess = 1 - n_train
            shrink_val = min(n_val, excess)
            n_val -= shrink_val; excess -= shrink_val
...
def evaluate_teacher(teacher_backend, density, poses, chunk: int):
    ...
    for s in range(0, n, chunk):
        ...
        if first_meta is None:
            # Preserve backend identity, forces_available, tile etc. from the
            # first chunk's TeacherResult (these are pose-independent).
            first_meta = dict(r.metadata)
        print(f"  [teacher] {e}/{n} ({100.0 * e / n:.0f}%)", flush=True)
    elapsed = time.perf_counter() - t0
    # Carry forward the backend metadata (backend, model_path, tile,
    # interaction_energy, forces_available …). Without this, downstream
    # consumers cannot tell which teacher produced the data, and the
    # force-mode auto-detect falls back to a brittle "any(F != 0)" heuristic.
    meta = dict(first_meta or {})
    meta.update({
...
def save_apples(out_dir: Path, poses, teacher, splits, sig, density,
    ...
    # IMPORTANT: persist adsorbate.charges too. The model's runtime energy
    # path checks ``poses.adsorbate.charges`` to decide whether the
    # fixed-charge Coulomb channel is active (model.py uses charges if
    # present). Dropping them silently rewrites the Hamiltonian on reload.
    ads_charges = poses.adsorbate.charges
    if ads_charges is None:
        ads_charges_arr = np.zeros(0, dtype=float)
        ads_charges_flag = False
    else:
        ads_charges_arr = np.asarray(ads_charges, dtype=float)
        ads_charges_flag = True
    teacher_forces_available = bool(teacher.metadata.get(
        "forces_available", bool(np.any(np.asarray(teacher.forces) != 0.0))
    ))
    np.savez_compressed(
        ...
        teacher_forces_available=np.bool_(teacher_forces_available),
        teacher_metadata=np.array(dict(teacher.metadata), dtype=object),
        ...
        adsorbate_charges=ads_charges_arr,
        adsorbate_has_charges=np.bool_(ads_charges_flag),
...
def load_cached_teacher(out_dir: Path, sig: str):
    ...
    cached_sig = str(d["signature"])
    if cached_sig != sig:
        import warnings as _w
        _w.warn(
            f"[cache] signature mismatch in {p}: cached={cached_sig!r}, "
            f"current={sig!r} — discarding cache. If you renamed inputs or "
            f"upgraded the signature scheme, this is expected (re-run teacher)."
        )
        return None
    # Restore as much teacher metadata as we have. Some early caches predate
    # forces_available; if absent, infer it from the actual force values.
    f_teacher = np.asarray(d["F_teacher"], dtype=float)
    if "teacher_forces_available" in d.files:
        fa = bool(d["teacher_forces_available"])
    else:
        fa = bool(np.any(f_teacher != 0.0))
    meta = {"loaded_from_cache": True, "forces_available": fa}
    if "teacher_metadata" in d.files:
        try:
            meta.update(dict(d["teacher_metadata"].item()))
        except Exception:
            pass
    return TeacherResult(
...
def main():
    ...
    print(f"[1/6] Loading density (kind={cfg.density_backend.kind})...", flush=True)
    density = make_density_backend(cfg.density_backend, grid=cfg.grid).load()
    print(f"   slab atoms={density.natoms}  cell={np.diag(density.cell).round(3)}",
          flush=True)
    # Hard sanity: ionic substrates MUST carry non-zero point charges.
    # An ionic xyz with neutral atoms reduces to a Pauli/London-only fit and
    # silently mis-represents the substrate.
    if args.substrate_class == "ionic":
        ch = density.charges
        if ch is None or not np.any(np.abs(np.asarray(ch, dtype=float)) > 1e-8):
            raise ValueError(
                f"--substrate-class ionic but the density backend returned "
                f"no/zero atomic charges. Ionic substrates require non-zero "
                f"point charges in the xyz (column 5). "
                f"density.charges = {ch!r}"
            )
    ...
    if args.load_apples:
        ...
        # Restore charges if present in the cache (fix: previously omitted, which
        # silently rewrote the Hamiltonian on reload because the runtime energy
        # path checks ``adsorbate.charges`` to enable fixed-charge Coulomb).
        ads_charges_loaded = None
        if "adsorbate_has_charges" in d.files and bool(d["adsorbate_has_charges"]):
            ads_charges_loaded = np.asarray(d["adsorbate_charges"], dtype=float)
        ...
        n = len(poses.positions)
        sig = _signature_from_cfg(poses, adsorbate.name, cfg)
    ...
    # ---- Outlier filter on loaded data too (skipped above when --load-apples)
    keep_mask = np.abs(teacher.energies) <= args.max_abs_teacher_eV
    n_dropped = int(np.sum(~keep_mask))
    if n_dropped:
        print(f"   [filter] dropping {n_dropped}/{len(teacher.energies)} poses "
              f"with |E| > {args.max_abs_teacher_eV} eV", flush=True)
        keep_idx = np.where(keep_mask)[0]
        old_to_new = -np.ones(len(teacher.energies), dtype=int)
        old_to_new[keep_idx] = np.arange(len(keep_idx))
        poses = slice_poses(poses, keep_idx)
    ...
    # ---- Energy-only DFT detection: if teacher has no forces (all zeros or
    #      forces_available=False in metadata), auto-switch the Stage 2 fit to
    #      energy-only. Important for users feeding pure DFT energies (no
    #      gradient calculation) — most common Psi4/Gaussian/Q-Chem output.
    teacher_has_forces = bool(teacher.metadata.get(
        "forces_available",
        bool(np.any(np.asarray(teacher.forces) != 0.0)),
    ))
    if not teacher_has_forces and args.fit_mode == "joint":
        print(f"   [auto] teacher provides no forces → switching --fit-mode joint → energy",
              flush=True)
        args.fit_mode = "energy"
    ...
    # Whether to install the raw 1/r⁶ basis for Stage 2 is class-dependent.
    recipe = get_substrate_class_recipe(args.substrate_class)
    use_raw_r6 = bool(recipe["use_raw_r6_stage2"])
    do_stage2 = bool(cfg.training.two_stage_c6 and cfg.training.fit_c6_coeff)
    if use_raw_r6:
        model.install_raw_r6_grid()
        print(f"   raw-1/r⁶ Stage-2 basis installed (substrate class '{args.substrate_class}')", flush=True)
    else:
        print(f"   raw-1/r⁶ Stage-2 basis SKIPPED (substrate class '{args.substrate_class}' "
              f"does not benefit from r⁻⁶ refinement)", flush=True)

    fit_t0 = time.perf_counter()
    if not do_stage2:
        # PLQ-only fit (no Stage 2). Used for ionic substrates by default.
        print("[fit] PLQ-only (Stage 1 gradient descent; no Stage 2)", flush=True)
        stage1 = fit_stage1_plq_only(density, train_poses, train_teacher, model, cfg.training)
        fitted_params = stage1.params
        stage1_history = stage1.history
        stage2_metrics = stage1.metrics
        joint_diag = None
    elif args.fit_mode == "joint":
    ...
    # ---- Export FireCore-compatible Bspline_PLQd.npy + sidecar files ----
    # The output of this script is meant to feed FireCore at MD time; we do
    # NOT swallow export failures (previously a try/except printed a warning
    # and continued). Fail loud so the user sees that no usable grid was
    # produced. If you need to investigate a failed export interactively,
    # set GRIDFF_JAX_EXPORT_LENIENT=1 in the environment.
    _shim = FitResult(params=fit_result_params, history=stage1_history, metrics=stage2_metrics)
    if _os.environ.get("GRIDFF_JAX_EXPORT_LENIENT", "0") == "1":
        try:
            export_paths = export_firecore_artifacts(
                out_dir=out_dir, density=density, model=model, fit_result=_shim,
                toggles=cfg.toggles, teacher_backend_id=cfg.teacher_backend.kind,
            )
            print(f"   exported: {export_paths['plq_path']}", flush=True)
        except Exception as _e:
            print(f"   [warn] FireCore export FAILED (LENIENT mode): {_e!r}", flush=True)
            export_paths = {"plq_path": None, "dispersion_present": False}
    else:
        export_paths = export_firecore_artifacts(
            out_dir=out_dir, density=density, model=model, fit_result=_shim,
            toggles=cfg.toggles, teacher_backend_id=cfg.teacher_backend.kind,
        ...
        print(f"   exported: {export_paths['plq_path']}", flush=True)
        if export_paths.get("dispersion_present"):
            print(f"   exported: {export_paths['dispersion_path']}", flush=True)
            print(f"   exported: {export_paths['dispersion_fit_path']}", flush=True)
            print(f"   [note] fitted student includes a C6 channel and/or "
                  f"non-zero energy_offset; FireCore PLQ grid alone does NOT "
                  f"reproduce it. Use the Dispersion + DispersionFit sidecars.",
                  flush=True)
    ...
    # only safe with a *live* teacher; the precomputed backend can't extrapolate.)
    slice_info = None
    if args.teacher_kind in ("madsurf", "generic_calc", "synthetic"):
        ...
        print(f"   [skip] slice_uv + channels_vs_z plots: teacher_kind="
              f"{args.teacher_kind!r} cannot evaluate new poses on demand", flush=True)
    ...
    with (out_dir / "report.txt").open("w") as fh:
        ...
        if slice_info is not None:
            ...
            fh.write(f"\nSlice corrugation: skipped (teacher_kind={args.teacher_kind!r} "
                     f"cannot evaluate new poses)\n")
