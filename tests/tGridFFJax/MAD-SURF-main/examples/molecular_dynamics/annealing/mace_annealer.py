...
nsteps_anneal = 1000 # number of steps taken for the annealing
nsteps_quench = 200 # number of steps per temperature increment (temperature_anneal-temperature_quench/|dT|)

temperature_anneal = 810  # K
temperature_quench = 10   # K
dT = -50                  # temperature step

friction = 0.01 / units.fs  # weak coupling

z_threshold = 2.5 # z-threshold for fixing atoms
fixed_atom_indices = [] # [287, 285] # if other atoms need fixing, define their indices
...
def write_frame():
    # === Logging and saving ===
    dyn.atoms.write(f"{save_file}_{temperature_anneal}K_anneal.xyz", append=True, format="extxyz")

dyn.attach(MDLogger(dyn, atoms, f"{save_file}_{temperature_anneal}K_anneal.log", header=True, stress=False,
