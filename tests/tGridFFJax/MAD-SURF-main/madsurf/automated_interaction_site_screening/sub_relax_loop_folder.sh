#!/bin/bash
#SBATCH --account=project_2008059
#SBATCH -p medium # test - for testing 1h ; medium - up to 20 nodes/36 hours ; large - 20-200 nodes/36 hours #
#SBATCH --time=1-12:00:00      # dd-hh:mm:ss
#SBATCH -J aiss_rlx_aims # name 
#SBATCH -o sbatch-%j.out # where the outputs & errors are written
#SBATCH -N 2                  # N nodes to run (N x 64 = n); max 192 ; max debug 48
#SBATCH --ntasks-per-node=128                # n processes to run (N x 64 = n); max 192 ; max debug 48

# check modules.txt for updating following lines: # 
#module purge
module load gcc/11.2.0 openmpi/4.1.2 openblas/0.3.18-omp netlib-scalapack/2.1.0 
export OMP_NUM_THREADS=1
ulimit -s unlimited



# Base directory where calc_* folders are located
base_dir="/scratch/project_2008059/mgonzalez/aims_calc_from_aiss"

# Loop through all calc_* folders in ascending order
for i in $(seq -w 0 824); do
    folder="$base_dir/calc_$i"
    if [ -d "$folder" ]; then
        echo "relaxing on folder: $folder"
        cd "$folder"
        srun /scratch/project_2008059/FHI-aims/build_211214/aims.211214.x > "$folder/acq.out" 
        
    fi
done
