#!/usr/bin/env python3
"""Run the strict-PLQ Ag z-scan benchmark on CHONH2 and transfer molecules."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run CHONH2/CO/H2O strict-PLQ Ag transfer benchmarks")
    parser.add_argument("--out-dir", default=str(ROOT / "tests/tGridFFJax/ag_transfer_suite"))
    parser.add_argument("--adsorbates", default="CHONH2,CO,H2O")
    ...
    parser.add_argument("--heldout-tilt-x-deg", type=float, default=20.0)
    parser.add_argument("--heldout-tilt-y-deg", type=float, default=10.0)
    parser.add_argument("--heldout-yaw-deg", type=float, default=0.0)
...
def _run_one(args, adsorbate: str, out_dir: Path):
    ...
    cmd = [
        ...
        "--heldout-tilt-x-deg",
        str(args.heldout_tilt_x_deg),
        "--heldout-tilt-y-deg",
        str(args.heldout_tilt_y_deg),
        "--heldout-yaw-deg",
        str(args.heldout_yaw_deg),
...
def main():
    ...
    for adsorbate, summary in summaries.items():
        aggregate[adsorbate] = {
            ...
            "heldout_primary_energy_rmse": summary.get("heldout_orientation", {}).get("windows", {}).get("primary", {}).get("all", {}).get("energy", {}).get("rmse"),
            "heldout_primary_force_rmse": summary.get("heldout_orientation", {}).get("windows", {}).get("primary", {}).get("all", {}).get("force", {}).get("rmse"),
        }
    result = {
    ...
    with (out_dir / "transfer_suite_summary.json").open("w", encoding="utf8") as fout:
