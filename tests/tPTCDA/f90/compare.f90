  ...
  do i = 1, natoms
     do j = 1, 3
        if ( abs(f_lammps(i,j)) > tiny(1.d0) ) then
           write(*,*) 'FORCE ', i, j, f_lammps(i,j), f_firecore(i,j), &
                abs(f_lammps(i,j)-f_firecore(i,j)), abs((f_lammps(i,j)-f_firecore(i,j))/f_lammps(i,j))
        else
           write(*,*) 'FORCE ', i, j, f_lammps(i,j), f_firecore(i,j), &
                abs(f_lammps(i,j)-f_firecore(i,j)), abs(f_firecore(i,j))
        end if
     end do
  end do

  if ( abs(e_lammps) > tiny(1.d0) ) then
     write(*,*) 'ENERGY ', e_lammps, e_firecore, abs(e_lammps-e_firecore), &
          abs((e_lammps-e_firecore)/e_lammps)
  else
        write(*,*) 'ENERGY ', e_lammps, e_firecore, abs(e_lammps-e_firecore), abs(e_firecore)
  end if
