...
def main():
    ap = argparse.ArgumentParser(description='Two-molecule STM GF(Dyson) kernel test (workgroup-per-pixel)')
    ...
    ap.add_argument('--outdir', default='export/stm_gf_dyson_2mol')
        ...
        for smp_ang in smp_ang_list:
            ...
            for tip_mo in tip_mo_list:
                for smp_mo in smp_mo_list:
                    ...
                    # Reference overlap-STM (existing code path) for comparison
                    ...
                    print(f"DEBUG Fortran 2mol GF tipMO={tip_mo+1} smpMO={smp_mo+1} E_tip={E_tip:+.6f} E_smp={E_smp:+.6f} E={E_transport:+.6f}")
                    t0 = time.time()
                    gf_I = fc.stm_gf_2mol_mo_2points(
                    ...
                    print(f"DEBUG Fortran 2mol GF time = {time.time()-t0:.3f} s")
                    gf_I = np.asarray(gf_I, dtype=np.float64).reshape(args.n, args.n)

                    gf_I_log = np.log10(gf_I + 1e-30)
                    ov_I_log = np.log10(ov_I + 1e-30)

                    fig, ax = plt.subplots(2, 3, figsize=(18, 12))
                    fig.suptitle(f"2mol GF(dyson_wg) vs orbitals: tip {tip_name} MO{tip_mo+1} smp {smp_name} MO{smp_mo+1} zmid={zmid:.2f} ztip={args.ztip:.2f} R_tip={args.tip_axis}{int(tip_ang)} R_smp={args.smp_axis}{int(smp_ang)}")
                    ...
                    im = ax[0,2].imshow(ov_I_log.T, origin='lower', extent=extent2, cmap='viridis', aspect='equal')
                    ax[0,2].set_title('3) overlap-STM log10(|t|^2)'); ax[0,2].set_xlabel('x (Å)'); ax[0,2].set_ylabel('y (Å)')
                    plot_atoms(ax[0,2], smp_pos, mol_smp.atypes)
                    fig.colorbar(im, ax=ax[0,2], fraction=0.046, pad=0.04)

                    im = ax[1,0].imshow(gf_I.T, origin='lower', extent=extent2, cmap='viridis', aspect='equal')
                    ax[1,0].set_title('4) Fortran GF current'); ax[1,0].set_xlabel('x (Å)'); ax[1,0].set_ylabel('y (Å)')
                    plot_atoms(ax[1,0], smp_pos, mol_smp.atypes)
                    fig.colorbar(im, ax=ax[1,0], fraction=0.046, pad=0.04)

                    im = ax[1,1].imshow((psi_s*psi_s).T, origin='lower', extent=extent2, cmap='viridis', aspect='equal')
                    ax[1,1].set_title('5) ψ_sample(mid)^2 (ref)'); ax[1,1].set_xlabel('x (Å)'); ax[1,1].set_ylabel('y (Å)')
                    plot_atoms(ax[1,1], smp_pos, mol_smp.atypes)
                    fig.colorbar(im, ax=ax[1,1], fraction=0.046, pad=0.04)

                    im = ax[1,2].imshow((gf_I_log - ov_I_log).T, origin='lower', extent=extent2, cmap='bwr', aspect='equal')
                    ax[1,2].set_title('6) log10(GF) - log10(overlap)'); ax[1,2].set_xlabel('x (Å)'); ax[1,2].set_ylabel('y (Å)')
                    plot_atoms(ax[1,2], smp_pos, mol_smp.atypes)
                    fig.colorbar(im, ax=ax[1,2], fraction=0.046, pad=0.04)

                    save_plot(fig, f"fortran_dyson2mol_tip{tip_name}_MO{tip_mo+1:03d}_smp{smp_name}_MO{smp_mo+1:03d}_zmid{zmid:.2f}_ztip{args.ztip:.2f}_tip{args.tip_axis}{int(tip_ang):03d}_smp{args.smp_axis}{int(smp_ang):03d}", export_dir=export_dir)
                    plt.close(fig)
