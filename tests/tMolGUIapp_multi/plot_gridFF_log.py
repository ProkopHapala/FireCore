...
data = np.loadtxt( 'checkSampleGridFF.log', skiprows=1 ).transpose()
...
plt.subplot(1,3,1); plt.title('GridFF vs NBFF'); plt.plot(x,E     ,label='E'); plt.plot(x,Fz,label='Fz'); plt.plot(x,Eref,':', label='Eref');  plt.plot(x,FzRef,':',label='FzRef'); plt.ylim(-0.2,0.2); plt.legend(); plt.grid()
# ---- Errr
