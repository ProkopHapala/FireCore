...
# Script to generate Morse/Pauli potential scan (handles both 1D and 2D scans)
import sys
import os
import numpy as np
import shutil
...
print(f"Initializing MMFF for MORSE/PAULI potential {scan_mode} scan...")
mmff.init(xyz_name=molecule, surf_name=substrate, bUFF=True, bSimple=True)
mmff.getBuffs()

# Set only Morse/Pauli components (zero out Coulomb)
print("Setting Morse/Pauli components only (zeroing out Coulomb)...")
mmff.setSwitches2(NonBonded=1, MMFF=1, SurfAtoms=1, GridFF=1)

# Zero out Coulomb (Q) components in PLQs
original_q = np.count_nonzero(mmff.PLQs[:,2])
mmff.PLQs[:,2] = 0.0
...
print(f"Zeroed out {original_q} Coulomb (Q) components")
...
if scan_mode == '1d':
    ...
    # Set output paths
    output_prefix = f"{output_dir}/{mol_name}_morse"
    
    # Run 1D scan
    print(f"Running 1D scan for Morse/Pauli potential...")
    scanPlot_uff(
        nscan=nscan,
        span=span,
        dir=dir_vec,
        p0=p0,
        label=f"{mol_name} Morse/Pauli Potential Scan",
        saveFig=f"{output_prefix}.png",
        saveData=f"{output_prefix}.dat"
    )
    print(f"Morse/Pauli 1D scan completed. Data saved in {output_prefix}.dat")
    ...
    # Set output paths
    output_prefix = f"{output_dir}/{mol_name}_morse_2d"
    
    # Run 2D scan
    print(f"Running 2D scan for Morse/Pauli potential...")
    scanPlot2D_uff(
        nscan1=nscan1,
        nscan2=nscan2,
        span1=span1,
        ...
        dir2=dir2,
        p0=p0,
        label=f"{mol_name} Morse/Pauli Potential 2D Scan",
        saveFig=f"{output_prefix}.png",
        saveData=f"{output_prefix}.npz",
        saveDataTxt=f"{output_prefix}.dat"
    )
    print(f"Morse/Pauli 2D scan completed. Data saved in {output_prefix}.npz")
