...
# profile_gridff_direct.sh - Direct GridFF OpenCL profiler wrapper
# Usage: ./profile_gridff_direct.sh [options]

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROFILER="${SCRIPT_DIR}/direct_gridff_profiler.py"
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"

# Check if the profiler exists
if [ ! -f "$PROFILER" ]; then
    echo "Error: Direct GridFF profiler not found at $PROFILER"
    exit 1
fi
