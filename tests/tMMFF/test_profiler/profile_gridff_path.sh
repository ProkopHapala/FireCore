...
# profile_gridff_path.sh - GridFF Path Profiler wrapper
# Usage: ./profile_gridff_path.sh [options]

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROFILER="${SCRIPT_DIR}/gridff_path_profiler.py"
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"

# Check if the profiler exists
if [ ! -f "$PROFILER" ]; then
    echo "Error: GridFF Path profiler not found at $PROFILER"
    exit 1
fi
