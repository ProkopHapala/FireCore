...
from pyBall.OCL.GridFF_profiled import kernel_times
...
print(f"Number of Kernels: {len(kernel_times)}")
...
results = {
    ...
    "kernels": [{"name": name, "times": times, "total_time": sum(times), "avg_time": sum(times) / len(times), "executions": len(times)} for name, times in kernel_times.items()]
}
...
# Plot kernel durations
import matplotlib.pyplot as plt
import numpy as np

# Extract kernel names and total times
...
# Save to file
plot_file = os.path.join(output_dir, "firecore_kernel_durations.png")
plt.savefig(plot_file)
print(f"Kernel duration plot saved to {plot_file}")
