...
log_summary="${OUT_DIR}/relax_summary_mmff.txt"
: >"${log_summary}"

for mol in "${molecules[@]}"; do
  base=$(basename "${mol}")
  out_xyz="${OUT_DIR}/traj_${base}.MMFF.xyz"
  echo "=== MMFF ${base} ==="
  "${PYTHON}" -u "${SCRIPT_DIR}/relax_MMFF_pyocl.py" \
    --molecule "${mol}" \
    --steps "${STEPS}" \
    --dt "${DT}" \
    --damp "${DAMP}" \
