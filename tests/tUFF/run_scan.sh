...
# # Run the Python script with the specified arguments for z-scan.
...
# # Run the Python script with the specified arguments for xy-scan.
...
# Run the Python script with the specified arguments for z-scan.
export Z_RANGE="0.0,10.0,0.02"
echo "Z_RANGE="$Z_RANGE
python3 -u scan_files/scan_MMFF.py --preset grid-only --grid-ff --nconf 1000 --scan_dim z --z_range "$Z_RANGE" --surf xyz/surfaces_for_throughput/NaCl_6x6_L3 --disable bonds --scan_pos "0.0,0.0" 2>&1 | tee OUT-UFF-z-scan
