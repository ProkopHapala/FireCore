#!/usr/bin/env node
/// @deprecated Use `node tests/tSiNCs/nanocrystals.mjs audit` instead. This script is kept for backward compatibility.
...
async function runGenPreset(genArgs, outDir) {
    ...
    const cmd = ['node', path.join(TEST_DIR, 'gen_nanocrystals.mjs'), '--samples', '1', '--seed', '42', '--maxFiles', '1', '--caps', 'H', '--insertProb', '0', '--collapseProb', '0', '--outDir', outDir, '--prefix', 'audit', ...genArgs];
...
async function main() {
    ...
    if (args.allPresets) {
        ...
        console.error('usage: node tests/tSiNCs/test_nanocrystal_geometry.mjs --preset G2 | --input file.mol2 | --all-presets [--outDir DIR]');
        process.exit(1);
    }
