...
function usage() {
    return `Usage: node tests/tSiNCs/nanocrystals.mjs <subcommand> [options]
...
Run 'node tests/tSiNCs/nanocrystals.mjs <subcommand> --help' for subcommand-specific options.`;
}
...
function generateUsage() {
    return `Usage: node tests/tSiNCs/nanocrystals.mjs generate [options]
...
function cmdGenerate(argv) {
    ...
    const args = genArgsFromConfig(nested, RES, { outDir: merged.outDir || path.join(TEST_DIR, 'OUT_nanocrystals'), prefix: merged.prefix || 'nc', seed: merged.seed ?? 42, samples: merged.samples ?? 1 });
...
function ensembleUsage() {
    return `Usage: node tests/tSiNCs/nanocrystals.mjs ensemble [options]
 
Options:
  --config PATH          ensemble JSON (default: tests/tSiNCs/ensemble.example.json)
  --data-dir PATH        per-crystal NPZ cache (default: OUT_nc_ensemble_v2/data)
  --output-dir PATH      accumulated outputs (default: OUT_nc_ensemble_v2/out)
  --work-dir PATH        scratch (default: OUT_nc_ensemble_v2/work)
...
function topologyUsage() {
    return `Usage: node tests/tSiNCs/nanocrystals.mjs topology [options]
...
function ringsUsage() {
    return `Usage: node tests/tSiNCs/nanocrystals.mjs rings [options]
...
function auditUsage() {
    return `Usage: node tests/tSiNCs/nanocrystals.mjs audit [options]
...
async function cmdAudit(argv) {
    ...
    if (merged.allPresets) {
        ...
        console.error('usage: node tests/tSiNCs/nanocrystals.mjs audit --preset G2 | --input file.mol2 | --all-presets [--out-dir DIR]');
        process.exit(1);
    }
...
function nonbondUsage() {
    return `Usage: node tests/tSiNCs/nanocrystals.mjs nonbond [options]
