#!/usr/bin/env node
/// @deprecated Use `node tests/tSiNCs/nanocrystals.mjs topology` instead. This script is kept for backward compatibility.
...
function parseArgs(argv) {
    const out = {
        input: null,
        outDir: path.join(TEST_DIR, 'linearized'),
        addAngle: true,
        addDihedral: true,
        htmlViewer: true,
        exportNpz: true,
    ...
    if (!out.input) throw new Error('Usage: node tests/tSiNCs/build_linearized_topology.mjs --input path.mol2|xyz [--out-dir dir] [--export-json 0|1] [--export-npz 0|1]');
    return out;
}
