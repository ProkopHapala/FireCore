#!/usr/bin/env node
/// @deprecated Use `node tests/tSiNCs/nanocrystals.mjs ensemble` instead. This script is kept for backward compatibility.
...
function usage() {
    return `Usage: node tests/tSiNCs/run_nanocrystal_ensemble.mjs [options]
...
function parseArgs(argv) {
    const out = {
        config: path.join(REPO, 'tests/tSiNCs/ensemble.example.json'),
        dataDir: path.join(TEST_DIR, 'OUT_nc_ensemble_v2/data'),
        outputDir: path.join(TEST_DIR, 'OUT_nc_ensemble_v2/out'),
        workDir: path.join(TEST_DIR, 'OUT_nc_ensemble_v2/work'),
        nCrystals: null,
...
function buildGenCli(genParams, outDir, seed) {
    const g = genParams;
    const args = [
        path.join(TEST_DIR, 'gen_nanocrystals.mjs'),
