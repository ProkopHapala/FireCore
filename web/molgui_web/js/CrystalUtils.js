...
export function applySymmetryOpsFracSites(sites, symOps, opts = {}) {
    ...
    for (const a of sites) {
        ...
        for (const op of symOps) {
            ...
            seen.add(key);
            out.push({ element: el, x: fx, y: fy, z: fz });
        }
    }
    return out;
}
...
export function cifToCrystalData(cifText) {
    ...
    return { lattice: lat, spg, symOps, sites };
}
...
export function genCrystalFromCIF(cifText, params = {}) {
    ...
    const cell = crystalSitesToCellData(crystal);
    let mol = null;
    if (params.slab) {
        ...
        mol = genReplicatedCellSlab({ lvec: cell.lvec, basisPos: cell.basisPos, basisTypes: cell.basisTypes, nRep, origin, nHat: n, cmin, cmax });
    } else {
        mol = genReplicatedCell({ lvec: cell.lvec, basisPos: cell.basisPos, basisTypes: cell.basisTypes, nRep, origin, buildBonds: !!params.buildBonds, mmParams: params.mmParams, bondTol: params.bondTol });
    }
...
function _parseMPWyckoffAtoms(mpJson) {
    const cs = mpJson && mpJson.crystal_structure;
    if (!cs) throw new Error('MP JSON missing crystal_structure');
    const spg = cs.symmetry && (cs.symmetry.international_number | 0);
    if (!(spg > 0)) throw new Error('MP JSON missing symmetry.international_number');
    if (!cs.atomic_positions || !cs.atomic_positions.length) throw new Error('MP JSON missing crystal_structure.atomic_positions');
    const atoms = cs.atomic_positions.map((a) => ({
        wyckoff: a.wyckoff,
        element: a.element,
        x: parseFraction(a.x),
        y: parseFraction(a.y),
    ...
    return { spg, atoms, meta: cs.structure_meta || null };
}

function _uniqueElements(atoms) {
    const set = new Set();
    for (const a of atoms) set.add(a.element);
    return Array.from(set);
}

function _expandSG225_Fm3m(atoms) {
    const out = [];
    const add = (el, x, y, z) => out.push({ element: el, x: wrapFrac(x), y: wrapFrac(y), z: wrapFrac(z) });
    const TF = [
        [0, 0, 0],
        [0.5, 0.5, 0],
        [0.5, 0, 0.5],
    ...
    for (const a of atoms) {
        const w = (a.wyckoff || '').toLowerCase();
        if (w === '4a') {
            for (const t of TF) add(a.element, 0 + t[0], 0 + t[1], 0 + t[2]);
        } else if (w === '8c') {
            const p0 = [a.x, a.y, a.z];
            const p1 = [a.x + 0.5, a.y + 0.5, a.z + 0.5];
            for (const t of TF) add(a.element, p0[0] + t[0], p0[1] + t[1], p0[2] + t[2]);
            for (const t of TF) add(a.element, p1[0] + t[0], p1[1] + t[1], p1[2] + t[2]);
        } else {
            throw new Error(`SG225: unsupported wyckoff '${a.wyckoff}' (only 4a,8c supported)`);
        }
    }
    return out;
}

function _expandSG227_Fd3m(atoms) {
    const els = _uniqueElements(atoms);
    if (els.length !== 1) throw new Error(`SG227: expected 1 element, got ${els.join(',')}`);
    const el = els[0];
    const out = [];
    const add = (x, y, z) => out.push({ element: el, x: wrapFrac(x), y: wrapFrac(y), z: wrapFrac(z) });
    const TF = [
        [0, 0, 0],
        [0.5, 0.5, 0],
        [0.5, 0, 0.5],
    ...
    for (const t of TF) add(t[0], t[1], t[2]);
    for (const t of TF) add(t[0] + 0.25, t[1] + 0.25, t[2] + 0.25);
    return out;
}

function _expandMPWyckoffLimited(spg, atoms) {
    if (spg === 225) return _expandSG225_Fm3m(atoms);
    if (spg === 227) return _expandSG227_Fd3m(atoms);
    throw new Error(`MP JSON: unsupported space group ${spg} (supported: 225, 227)`);
}

export function mpJsonToCellData(mpJson, opts = {}) {
    const { spg, atoms: wyckAtoms } = _parseMPWyckoffAtoms(mpJson);
    const lat = opts.lattice || null;
    if (!lat) throw new Error('mpJsonToCellData: opts.lattice is required (a,b,c,alpha,beta,gamma)');
    const lvec = latticeVectorsFromParams(lat);
    const fullFrac = _expandMPWyckoffLimited(spg, wyckAtoms);
    const n = fullFrac.length | 0;
    const basisPos = new Float32Array(n * 3);
    const basisTypes = new Uint8Array(n);
    const tmp = new Vec3();
    for (let i = 0; i < n; i++) {
    ...
    return { spg, lvec, basisPos, basisTypes };
}

export function genCrystalFromMPJson(mpJson, params = {}) {
    const nRep = (params.nRep !== undefined) ? params.nRep : [1, 1, 1];
    const origin = (params.origin !== undefined) ? params.origin : new Vec3(0, 0, 0);
    const cell = mpJsonToCellData(mpJson, { lattice: params.lattice });
    let mol = null;
    if (params.slab) {
        if (params.buildBonds) throw new Error('genCrystalFromMPJson: buildBonds not supported with slab cut');
        const hkl = params.slab.hkl;
        if (!hkl || hkl.length < 3) throw new Error('genCrystalFromMPJson: slab.hkl must be [h,k,l]');
        const cmin = +params.slab.cmin;
        const cmax = +params.slab.cmax;
        if (!(cmax > cmin)) throw new Error('genCrystalFromMPJson: slab requires cmax>cmin');
        const b = reciprocalLattice(cell.lvec);
        const n = new Vec3().setLincomb3(hkl[0] | 0, b[0], hkl[1] | 0, b[1], hkl[2] | 0, b[2]);
        const ln = n.normalize();
        if (!(ln > 0)) throw new Error('genCrystalFromMPJson: slab normal is zero');
        mol = genReplicatedCellSlab({ lvec: cell.lvec, basisPos: cell.basisPos, basisTypes: cell.basisTypes, nRep, origin, nHat: n, cmin, cmax });
    } else {
        mol = genReplicatedCell({ lvec: cell.lvec, basisPos: cell.basisPos, basisTypes: cell.basisTypes, nRep, origin, buildBonds: !!params.buildBonds, mmParams: params.mmParams, bondTol: params.bondTol });
    }
    mol.lvec = [cell.lvec[0].clone().mulScalar(nRep[0] | 0), cell.lvec[1].clone().mulScalar(nRep[1] | 0), cell.lvec[2].clone().mulScalar(nRep[2] | 0)];
    return { mol, lvec: mol.lvec, spg: cell.spg };
}

export function genReplicatedCellSlab(params = {}) {
    const { lvec, basisPos, basisTypes, nRep = [1, 1, 1], origin = new Vec3(0, 0, 0), nHat = null, cmin = 0.0, cmax = 0.0 } = params;
...
export function genReplicatedCell(params = {}) {
    const { lvec, basisPos, basisTypes, nRep = [1, 1, 1], origin = new Vec3(0, 0, 0), buildBonds = false, mmParams = null, bondTol = 0.2 } = params;
        ...
        for (let iy = 0; iy < nb; iy++) {
            for (let ix = 0; ix < na; ix++) {
                ...
                for (let ib = 0; ib < nBasis; ib++) {
                    ...
                    const id = mol.addAtom(s.x + x, s.y + y, s.z + z, Z);
                    if (ids) {
