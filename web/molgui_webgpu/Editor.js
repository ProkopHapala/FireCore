...
export class Editor {
    constructor(scene, camera, renderer, system, molRenderer) {
        ...
        this.gizmo.getHelper().visible = false; // Hidden by default until selection
        ...
        this.gizmo.addEventListener('dragging-changed', (event) => {
            ...
            if (window.app && window.app.controls) window.app.controls.enabled = !isDragging;
    ...
    isPointerOnGizmo(e) {
        if (!this.gizmo || !this.gizmo.getHelper().visible || !this.gizmo.enabled) return false;
        const rect = this.renderer.domElement.getBoundingClientRect();
        const ndcX = ((e.clientX - rect.left) / rect.width) * 2 - 1;
        const ndcY = -((e.clientY - rect.top) / rect.height) * 2 + 1;
        this.mouse.set(ndcX, ndcY);
        this.raycaster.setFromCamera(this.mouse, this.camera);
        const hits = this.raycaster.intersectObject(this.gizmo.getHelper(), true);
